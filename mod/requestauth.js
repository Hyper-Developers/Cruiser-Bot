module.exports = async (client) => {
  let sendReminder = async (member) => {
    if (
      ...
      return;
    await member.send({
      content: `emotehere \`${member.guild.name.replace(
        "`",
        "\\`"
      )}\` uses <@!${
        client.user.id
      }>'s automatic Anti-Nuke, which re-adds you in the case that you are falsely banned during a raid. Please click the button below to authorize <@!${
        client.user.id
      }>.`,
      components: [
        {
    ...
    await client.antinukeReminderCooldown.set(
      member.id,

        ...
        switch (interaction.options.getSubcommand()) {
          case "autoslowmode":
            if (interaction.options.get("target") == null) {
              ...
              await client.targetRatelimits60s.set(
                interaction.channelId,
                interaction.options.get("target").value
              );
            }
            break;
          case "autolockdown":
            if (interaction.options.get("threshold") == null) {
              ...
              await client.maximumRatelimits3s.set(
                interaction.channelId,
          ...
          case "virustotal":
            if (interaction.options.get("apikey") == null) {
              ...
              await client.virustotalApikeys.set(
                interaction.guild.id,
                interaction.options.get("apikey").value
              );
            }
            break;
        }
        return await interaction.reply({
          content: "Set option succesfully.",
          ephemeral: true,
        });
      }
    }

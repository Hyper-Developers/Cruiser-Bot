module.exports = async (client) => {
  ...
  client.on("messageCreate", async (msg) => {
    ...
    if (
      ...
      if (
        maximumRatelimits <= messagesLast3s[channel.id] &&
        channel
          ...
          channel.bulkDelete(
            lastMsgs.filter(
              (m) =>
                ...
                (!channel.permissionsFor(m.member) ||
                  !channel.permissionsFor(m.member).has("MANAGE_MESSAGES"))
            )
          );
        }
        ...
        await channel.send(
          "Automatic lockdown for " +
            5 * Math.pow(2, lockdownExp) +
            " seconds triggered by influx of messages"
        );
        setTimeout(async () => {
          await channel.permissionOverwrites.edit(
            channel.guild.roles.everyone,
